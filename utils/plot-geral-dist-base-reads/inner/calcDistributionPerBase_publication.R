...
ggplot()+ theme_classic(base_size=20)+ geom_bar(data=b1,aes(size,value,fill=variable),stat="identity",width=.8) + geom_bar(data=b2,aes(size,value,fill=variable),width=.8,stat="identity") + ylim(-m,m) + xlab("small RNA length (nt)") +ylab(paste("Number of small RNAs \n(",args[5],")",sep="")) + theme(panel.border = element_blank())+ theme(axis.line = element_line(color = 'black')) +

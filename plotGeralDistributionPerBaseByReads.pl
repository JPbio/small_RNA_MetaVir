...
GetOptions ("sam=s" => \$sam,"pattern!" => \$pattern,"profile!" => \$profile,"p=s" => \$prefix,"s=i" => \$start,"e=i" => \$end,"norm=i" => \$norm,"m=i" => \$minimo,
	"h!" => \$help,"keep!" => \$keep,"plot!" => \$plot,"antisense!" => \$antisense ,"revcomp!" => \$revcomp , "base!" => \$base);
 ...
 if(/^\w+/){
   ...
   if(not exists $read{$campos[0]}){
    ...
    ###count miRNA preference
  	...
  	if(not exists($base_total{$first})){$base_total{$first}=0;}
    ...
    if($size>=20 && $size <=23){
         if(not exists($base_si{$c}{$first})){$base_si{$c}{$first}=0;}
     ...
     if($size>=24 && $size <=30){
     	if(not exists($base_pi{$c}{$first})){$base_pi{$c}{$first}=0;}
...
open(O4,">$prefix.Geral_base_distribution_norm");  
	...
	for(my $i=$start;$i<=$end;$i++){   
	    ...
	    $ta=norm($base_per_size{$i}{"+"}{"A"});
		$tc=norm($base_per_size{$i}{"+"}{"C"});
		$tg=norm($base_per_size{$i}{"+"}{"G"});
		$tt=norm($base_per_size{$i}{"+"}{"T"});
		
	
		$nta=norm($base_per_size{$i}{"-"}{"A"} * -1);
		$ntc=norm($base_per_size{$i}{"-"}{"C"} * -1);
		$ntg=norm($base_per_size{$i}{"-"}{"G"} * -1);
		$ntt=norm($base_per_size{$i}{"-"}{"T"} * -1);
...
if(defined($plot)){
    if(defined($norm)){
       `R --no-save $prefix.Geral_base_distribution_norm $prefix.NORMALIZED_Geral_base_distribution_by_reads RPM < $PATH/calcDistributionPerBase_publication.R`;
	`R --no-save $prefix.Geral_base_distribution_norm $prefix.NORMALIZED_Geral_base_distribution_by_reads_ZOOM_piRNAS RPM < $PATH/calcDistributionPerBase_publication_zoom_piRNAs.R`;
       `R --no-save $prefix.Geral_base_distribution_norm $prefix.NORMALIZED_Geral_base_distribution_sumstrands_by_reads RPM < $PATH/calcDistributionPerBaseSumStrand_publication.R`;	
     
     }
    
    `R --no-save $prefix.Geral_base_distribution $prefix.Geral_base_distribution_by_reads < $PATH/calcDistributionPerBase_publication.R`;
    `R --no-save $prefix.Geral_base_distribution $prefix.Geral_base_distribution_by_reads_ZOOM_piRNAs < $PATH/calcDistributionPerBase_publication_zoom_piRNAs.R`;
    `R --no-save $prefix.Geral_base_distribution $prefix.Geral_base_distribution_sumstrands_by_reads < $PATH/calcDistributionPerBaseSumStrand_publication.R`;

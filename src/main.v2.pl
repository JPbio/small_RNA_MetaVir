...
use constant GAMBIARRA_BOWTIE_PATH => "/srna_metavir/asset/bowtie";

my $cmd_bowtie = GAMBIARRA_BOWTIE_PATH."/bowtie";
my $cmd_bowtie_build = GAMBIARRA_BOWTIE_PATH."/bowtie-build";
...
#     my $exec3 = "$cmd_bowtie $large_index -f -S -k 1 -p $process -v 1 --un $path_04_unmap_vector_reads $hostgenome $path_02_trim_filtered_gt15 | awk -F'\\t' '{if( \$2 != 4) print \$0}' > $path_03_map_host_sam  2>mapping_host.stats  ";
...
my $path_07_bN_analize_viral_header = "$path_07_bN_analyse_blastn_prefix.virus.contigs.header.fasta"; # $step7/contigs.bN.blastn.analyze.virus.contigs.header.fasta"
...
my $path_07_bN_analize_non_viral_header = "$path_07_bN_analyze_prefix.nonviral.contigs.header.fasta"; # "$step7/contigs.bN.analyze.nonviral.contigs.header.fasta";
...
my $path_07_dmnd_non_viral_header = "$step7/diamond_blastx_NonViral.header.fasta";
...
# #######################################################################
# ### Align viral, non viral and no hits against unmapped contigs -------
# #######################################################################

# $step_name = "Align viral, non viral and no hits against unmapped contigs";
...
my $path_11_smallrna_profiles_viral = "$step11/smallrna_profiles_viral";
my $path_11_smallrna_profiles_non_viral = "$step11/smallrna_profiles_non_viral";
my $path_11_smallrna_profiles_no_hits = "$step11/smallrna_profiles_no_hits";

if (not -e $path_11_smallrna_profiles_viral) {
    `mkdir $path_11_smallrna_profiles_viral`;
}
if (not -e $path_11_smallrna_profiles_non_viral) {
    `mkdir $path_11_smallrna_profiles_non_viral`;
}
if (not -e $path_11_smallrna_profiles_no_hits) {
    `mkdir $path_11_smallrna_profiles_no_hits`;
}
...
# `$cmd_bowtie_build $path_09_seq_viral_hit_no_hit $path_09_seq_viral_hit_no_hit`;

# print "\t#Mapping reads against viral hits(blastn and diamond) and nohits\n";
# my $exec10_13 = "$cmd_bowtie -f -S -p $process -v 1 $path_09_seq_viral_hit_no_hit $path_04_unmap_vector_bacters 2>>$path_warns | awk -F'\t' '{if( \$2 != 4) print \$0}' > $step9/reads_vs_contigsHitNoHit.v1.sam ";
...
#     `$cmd_bowtie_build $path_contig_viral_hits_all $path_contig_viral_hits_all`;

# } else {
#     print "\nNo contig viral hits found...\n";
# }

# `$cmd_bowtie -f -S -k 1 -p $process -v 1  $path_contig_viral_hits_all $step2/trimmed_filtered_gt15.fasta | awk -F '\t' '{if( \$2 != 4) print \$0}'  > $step10/reads.VS.contigs_hit_virus_blast.sam`;
...
# `$cmd_bowtie -f -S -k 1 -p $process -v 1  $step9/seq_ViralHits_and_NoHits.fasta $step2/trimmed_filtered_gt15.fasta | awk -F '\t' '{if( \$2 != 4) print \$0}'  > $step10/reads.VS.contigs_virus_and_nohit.sam`;
...
# `$cmd_bowtie_build $step10/reads.VS.contigs_virus_and_nohit.withSiRNA_and_PiRNA.fasta $step10/reads.VS.contigs_virus_and_nohit.withSiRNA_and_PiRNA.fasta > /dev/null `;
# `$cmd_bowtie_build $step10/reads.VS.contigs_virus_and_nohit.withSiRNA.fasta $step10/reads.VS.contigs_virus_and_nohit.withSiRNA.fasta > /dev/null `;
...
# `$cmd_bowtie -f -S -k 1 -p $process -v 1 $step10/reads.VS.contigs_virus_and_nohit.withSiRNA.fasta $step2/trimmed_filtered_gt15.fasta | awk -F '\t' '{if( \$2 != 4) print \$0}'  > $step10/reads.VS.contigs_virus_and_nohit.siRNAs.sam`;
# `$cmd_bowtie -f -S -k 1 -p $process -v 1  $step10/reads.VS.contigs_virus_and_nohit.withSiRNA_and_PiRNA.fasta $step2/trimmed_filtered_gt15.fasta | awk -F '\t' '{if( \$2 != 4) print \$0}'  > $step10/reads.VS.contigs_virus_and_nohit.siRNAs_and_piRNAs.sam`;

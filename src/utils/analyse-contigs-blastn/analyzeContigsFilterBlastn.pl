...
my $path_inner = "$path_utils/analyse-contigs-blastn/inner";
my $path_analyse_contigs_blastn_r = "$path_inner/analyzeContigsFilterBlastn.R";
...
if (defined($image)) {
   ...
   if (defined($debug)) { print "\n\n Downloading genomes..."; }
    ...
    foreach $s (keys %blast) {
        ...
        $c3="R --no-save $out $s $size $prefix $org_name < $path_analyse_contigs_blastn_r";

...
if (defined($profile)) {
    ...
    foreach $c (keys %contig) {
        ...
        if ($counting >= $minimo) {
				...
				$f0 = "$path_filter_sam_bowtie -i $cp.sam -si 20 -se 23 -o $cp.20to23.sam";
				$f1 = "$path_filter_sam_bowtie -i $cp.sam -si 21 -se 21 -o $cp.21.sam";
				$f2 = "$path_filter_sam_bowtie -i $cp.sam -si 24 -se 30 -o $cp.24to29.sam";
				$f3 = "$path_filter_sam_bowtie -i $cp.sam -si 22 -se 22 -o $cp.22to22.sam";
				$f4 = "$path_filter_sam_bowtie -i $cp.sam -si 15 -se 19 -o $cp.15to19.sam";
                ...
                # print $LOG_FH "$f1\n$f2\n";
                $comm0 = "$path_calc_density_base -sam $cp.20to23.sam -pace $pace -ref ".$contig{$c}."  -out  $cp.20to23.density";
                $comm = "$path_calc_density_base -sam $cp.sam -pace $pace -ref ".$contig{$c}."  -out  $cp.density";
                $comm2 = "$path_calc_density_base -sam $cp.21.sam -pace $pace -ref ".$contig{$c}."  -out $cp.21.density";
                $comm3 = "$path_calc_density_base -sam $cp.24to29.sam -pace $pace -ref ".$contig{$c}."  -out $cp.24to29.density";
                $comm4 = "$path_calc_density_base -sam $cp.22to22.sam -pace $pace -ref ".$contig{$c}."  -out $cp.22to22.density";
                $comm5 = "$path_calc_density_base -sam $cp.15to19.sam -pace $pace -ref ".$contig{$c}."  -out $cp.15to19.density";
